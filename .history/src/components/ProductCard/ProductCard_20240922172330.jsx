      ...
      try {
        ...
        if (Array.isArray(response.data)) {
          ...
          const newBooks = response.data.filter((book) => {
            // Kiểm tra nếu bookId đã tồn tại trong dữ liệu hiện tại
            return !data.some(
              (existingBook) => existingBook.bookId === book.bookId
            );
          });

          // Cập nhật dữ liệu với các sách mới không bị trùng lặp
          setData((prevData) => [...prevData, ...newBooks]);
        } else {
